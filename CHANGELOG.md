...
- **Large layouts draw at interactive speed.** Three changes in the viewer,
  which arrive with the move to gds-lens 1.4.0:

  A cell is GPU-instanced on what instancing saves rather than on how often it
  is placed. The old rule instanced anything placed eight times, which costs a
  draw call per cell per layer in every frame -- and a layout generated by
  gdsfactory or kfactory is thousands of distinct small cells placed a dozen
  times each, so it put tens of thousands of draw calls into every frame. A
  synthetic test chip of 2.5 million polygons went from 108 ms a frame to under
  1 ms, and from 4.35 s to load to 1.27 s. A cell placed a hundred thousand
  times still instances.

  Panning and zooming a very large layout reprojects the last render rather
  than redrawing the geometry, so a drag runs at the display's refresh rate
  with one real render when the camera stops. A 127-million-polygon layout went
  from 606 ms a frame to 8.4 ms while dragging. The frame the view settles on
  is identical to the one drawn without it. Merge mode and the difference
  highlight are not covered.

  Per-batch GPU state moved into vertex array objects built once at load.

- **The debug readout counts the frame's draw calls** where it used to say
  `no culling` -- the number that says whether a slow frame is spending its
  time on draw calls or on geometry. **GDSLens: Toggle Debug Tools**.
